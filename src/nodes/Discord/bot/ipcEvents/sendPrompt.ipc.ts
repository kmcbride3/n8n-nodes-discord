...
export default async function (process: NodeJS.Process) {
  process.on('message', async (message: { type: string; data: IDiscordNodePromptParameters }) => {
        ...
        if (state.ready) {
          ...
          state.client.channels
            ...
            .then(async (channel: Channel | null) => {
              ...
              addLog(`send:prompt to ${channelId}`, state.client)

              const promptProcessing = async (message: Message) => {
                state.promptData[message.id] = nodeParameters
                await pollingPromptData(message, nodeParameters.content, nodeParameters.timeout, state.client).catch(
                  (e: any) => addLog(`${e}`, state.client),
                )
                if (process.send) {
                  process.send({
                    type: 'send:prompt',
                    data: state.promptData[message.id],
                  })
                }
            ...
            .catch((e: any) => {
              addLog(`${e}`, state.client)
              if (process.send) {
                process.send({ type: 'send:prompt', data: false })
              }
            })
        }
      } catch (e) {
