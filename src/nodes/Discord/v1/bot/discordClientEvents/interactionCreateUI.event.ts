...
async function checkTriggerRolePermissions(
  ...
  if (interaction.isRepliable()) {
    await interaction
      ...
      .catch((e: Error) => handleNonFatalError('Failed to send permission denied reply', client, e, 'warn'))
  }
...
async function executeTriggerWorkflow(
  ...
  await interaction
    .deferUpdate()
    .catch((e: Error) => handleNonFatalError('Failed to defer interaction update', client, e, 'warn'))
...
async function checkPromptPermissions(
  ...
  if (promptData.restrictToRoles && Array.isArray(promptData.mentionRoles)) {
    ...
    if (!hasRole) {
      await interaction
        ...
        .catch((e: Error) => handleNonFatalError('Failed to send role restriction reply', client, e, 'warn'))
      return false
    }
  }
  ...
  if (promptData.restrictToTriggeringUser && triggeringUserId && interaction.user.id !== triggeringUserId) {
    await interaction
      ...
      .catch((e: Error) => handleNonFatalError('Failed to send user restriction reply', client, e, 'warn'))
    return false
  }
...
async function updatePromptUI(
  ...
  await interaction
    .update({ components: [] })
    .catch((e: Error) => handleNonFatalError('Failed to remove prompt components', client, e, 'warn'))
  ...
  await (channel as TextChannel)
    ...
    .catch((e: Error) => handleNonFatalError('Failed to send confirmation message', client, e, 'warn'))
  ...
  setTimeout(async () => {
    try {
      ...
      await message
        ...
        .catch((e: Error) => handleNonFatalError('Failed to edit prompt message', client, e, 'warn'))
    } catch (e) {
