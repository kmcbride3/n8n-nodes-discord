...
/**
 * V1 Legacy IPC Message Handler
 *
 * NOTE: This is V1 legacy IPC implementation.
 * V2 operations use direct Discord.js REST API calls following built-ins first architecture.
 * Prefer V2 operations for new implementations which minimize IPC usage.
 */
...
async function handlePlaceholderUpdate(
  ...
  if (message?.edit) {
    // Note: This retry logic is for WORKFLOW STATE synchronization (placeholder management),
    // NOT for Discord API retries. Discord.js handles API retries automatically.
    // This waits for the workflow execution state to be ready before editing the message.
    let retryCount = 0
    const retry = async () => {
      const placeholderId = executionMatching.placeholderId
      if (placeholderId && state.placeholderWaiting.get(placeholderId) && retryCount < 10) {
        ...
        // Discord.js message.edit() handles its own retries and rate limiting
