/**
 * ListRoles Operation
 * Fetches multiple roles from a guild
 */
...
interface IGET_ListRolesCredentials extends IV2DiscordCredentials {
  client: Client
}

export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
  return executeV2OperationWithClient<IGET_ListRolesCredentials>(this, {
    ...
    operation: async (ctx, { client }, itemIndex) => {
      ...
      // Fetch all roles (Discord.js doesn't support limit for roles)
      const collection = await guild.roles.fetch()

      // Convert to array and optionally simplify
      const items = collectionToArray(collection, simplify ? simplifyRole : undefined)
