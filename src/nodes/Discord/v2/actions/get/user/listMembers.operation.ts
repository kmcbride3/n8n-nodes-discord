 ...
 * Fetches multiple members from a guild
 */
...
interface IGET_ListMembersCredentials extends IV2DiscordCredentials {
  client: Client
}

export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
  return executeV2OperationWithClient<IGET_ListMembersCredentials>(this, {
    ...
    operation: async (ctx, { client }, itemIndex) => {
      ...
      // Fetch collection using Discord.js
      const collection = await guild.members.fetch({ limit })

      // Convert to array and optionally simplify
      const items = collectionToArray(collection, simplify ? simplifyMember : undefined)
