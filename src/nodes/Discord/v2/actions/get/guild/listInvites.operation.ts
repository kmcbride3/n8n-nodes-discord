 ...
 * Fetches multiple invites from a guild
 */
...
interface IGET_ListInvitesCredentials extends IV2DiscordCredentials {
  client: Client
}

export async function execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
  return executeV2OperationWithClient<IGET_ListInvitesCredentials>(this, {
    ...
    operation: async (ctx, { client }, itemIndex) => {
      ...
      // Fetch all invites (Discord.js doesn't support limit for invites)
      const collection = await guild.invites.fetch()

      // Convert to array and optionally simplify
      const items = collectionToArray(collection, simplify ? simplifyInvite : undefined)
